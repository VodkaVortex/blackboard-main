...
class LocalBackend:
    ...
    Each project gets a separate working directory under ``workspace_root``, unless
    ``working_directory`` selects a shared checkout for development. Neither mode is
    a filesystem sandbox. Worker processes inherit the host
    ...
    def __init__(self, config: LocalConfig):
        ...
        self._working_directory = (
            Path(config.working_directory).expanduser().resolve() if config.working_directory else None
        )
        if self._working_directory is not None and not self._working_directory.is_dir():
            raise ValueError(f"working_directory does not exist: {self._working_directory}")
