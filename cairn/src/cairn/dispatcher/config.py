...
WorkerType = Literal["claudecode", "codex", "pi", "pi_text", "pi_dev", "mock"]
...
class LocalConfig(BaseModel):
    ...
    def validate_working_directory(self) -> "LocalConfig":
        if self.working_directory is not None:
            if not self.working_directory.strip():
                raise ValueError("working_directory must not be empty")
            if self.workspace_root is not None or self.completed_action != "keep":
                raise ValueError("working_directory requires completed_action=keep and no workspace_root")
        return self
...
class DispatchConfig(BaseModel):
    ...
    def validate_execution_mode(self) -> "DispatchConfig":
        if any(worker.type == "pi_dev" for worker in self.workers):
            if self.runtime.execution != "local" or self.runtime.prompt_group != "development":
                raise ValueError("pi_dev requires local execution and the development prompt group")
        if any(worker.type == "pi_text" for worker in self.workers):
            if self.runtime.execution != "local" or self.runtime.prompt_group != "text":
                raise ValueError("pi_text requires local execution and the text prompt group")
