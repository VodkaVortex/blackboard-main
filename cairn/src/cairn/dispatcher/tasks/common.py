...
TEXT_REJECTION_CREATOR = "dispatcher.pi_text.rejection"
PI_DEV_REJECTION_CREATOR = "dispatcher.pi_dev.rejection"
LOG = logging.getLogger(__name__)


def report_text_rejection(client: CairnClient, project_id: str, worker: WorkerConfig, payload: dict) -> None:
    """Surface a conversational worker's rejection instead of retrying identical input."""
    if worker.type not in ("pi_text", "pi_dev"):
        return
    reason = payload.get("reason")
    if not isinstance(reason, str) or not reason.strip():
        reason = "Pi 无法根据当前输入完成本轮任务，未提供具体原因。"
    message = f"{worker.name}: {reason.strip()[:4000]}"
    creator = PI_DEV_REJECTION_CREATOR if worker.type == "pi_dev" else TEXT_REJECTION_CREATOR
    notice = client.create_hint(project_id, message, creator)
    if not notice.ok:
        LOG.warning("text rejection notice failed project=%s status=%s", project_id, notice.status_code)
    result = client.stop_project(project_id)
    if not result.ok and result.status_code != 409:
        LOG.warning("text rejection pause failed project=%s status=%s", project_id, result.status_code)
